//#region node_modules/archipelago.js/dist/index.js
...
var DataPackageManager = class {
	...
	async fetchPackage(games = [], update = true) {
		...
		if (this.#cache) {
			...
			for (const game of games) {
				...
				if (cachedPackage) data2.games[game] = cachedPackage;
				else notFoundGames.push(game);
			}
			if (update) this.importPackage(data2);
			games = notFoundGames;
		}
		const data = { games: {} };
		for (const game of games) {
	...
	exportPackage() {
		return { games: [...this.#packages.entries()].reduce((games, [game, pkg]) => {
	...
	lookupItemName(game, id, fallback = true) {
		...
		if (!gamePackage) return fallback ? fallbackName : void 0;
		const name = gamePackage.reverseItemTable[id];
		if (fallback && name === void 0) return fallbackName;
		return name;
	}
...
var DataStorageManager = class {
	...
	async fetch(input, monitor = false) {
		...
		if (monitor) {
			const monitorKeys = keys.filter((key) => this.#storage[key] === void 0);
...
var DeathLinkManager = class extends EventBasedManager {
	...
	sendDeathLink(source, cause) {
		...
		this.#lastDeath = Math.ceil(Date.now() / 1e3);
		const deathLink = {
			source,
			cause,
			time: this.#lastDeath
	...
	constructor(client) {
		...
		this.#client.socket.on("receivedItems", (packet) => {
			...
			this.#client.storage.notify([`_read_hints_${this.#client.players.self.team}_${this.#client.players.self.slot}`], this.#receivedHint.bind(this)).then((data) => {
				...
				for (const hint of hints) this.#hints[`${hint.finding_player}-${hint.location}`] = new Hint(this.#client, hint);
...
var MessageManager = class extends EventBasedManager {
	...
	async say(text) {
		...
		this.#client.socket.send(request);
		if (text.startsWith("!admin ")) await this.wait("adminCommand", (message) => message === text);
		else await this.wait("chat", (message) => message === text);
	}
	#onPrintJSON(packet) {
		const nodes = [];
		...
		switch (packet.type) {
			...
			case "Countdown": this.emit("countdown", [
				text,
				packet.countdown,
				nodes
			]);
...
var Player = class {
	...
	async fetchHints() {
		return (await this.#client.storage.fetch(`_read_hints_${this.team}_${this.slot}`)).map((hint) => new Hint(this.#client, hint));
	}
	get #networkSlot() {
...
var PlayersManager = class extends EventBasedManager {
	...
	findPlayer(slot, team) {
		if (team === void 0) team = this.#client.players.self.team;
		if (this.#players[team]) return new Player(this.#client, this.#players[team][slot]);
	}
};
var RoomStateManager = class extends EventBasedManager {
	...
	get hintCost() {
		if (this.hintCostPercentage > 0) return Math.max(1, Math.floor(this.hintCostPercentage * this.allLocations.length * .01));
		return 0;
	}
...
var SocketManager = class extends EventBasedManager {
	...
	async connect(url) {
		...
		if (typeof url === "string") {
			if (!/^([a-zA-Z]+:)\/\/[A-Za-z0-9_.~\-:]+/i.test(url)) try {
...
var Client = class {
	...
	constructor(options) {
		...
		else this.options = { ...defaultClientOptions };
	...
	async login(url, name, game = "", options) {
		...
		else this.#arguments = { ...defaultConnectionOptions };
...
//#endregion
export { exports_api as API, ArgumentError, BaseMessageNode, Client, ColorMessageNode, DataPackageManager, DataStorageManager, DeathLinkManager, EventBasedManager, Hint, IntermediateDataOperation, Item, ItemMessageNode, ItemsManager, LocationMessageNode, LoginError, MessageManager, PackageMetadata, Player, PlayerMessageNode, PlayersManager, RoomStateManager, SocketError, SocketManager, TextualMessageNode, UnauthenticatedError, clientStatuses, defaultClientOptions, defaultConnectionOptions, itemClassifications, itemsHandlingFlags, libraryVersion, permissions, slotTypes, targetVersion };

//# sourceMappingURL=archipelago__js.js.map